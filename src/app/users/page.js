...
// สีพื้นหลัง avatar ไล่ตามตัวอักษรแรกของชื่อ ให้แต่ละคนดูมีเอกลักษณ์
...
function Avatar({ firstname }) {
  ...
  return (
    <div
      className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${bg} ${text} font-semibold text-sm`}
    >
      {initial}
    </div>
  );
...
export default function UsersPage() {
  ...
  const handleEdit = (id) => {
    // TODO: เชื่อมต่อกับหน้าแก้ไขข้อมูลสมาชิก
    console.log("edit", id);
  };
      ...
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 p-6 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            ลองตรวจสอบการเชื่อมต่อแล้วลองใหม่อีกครั้ง
          </p>
        </div>
        <button
          onClick={fetchUsers}
